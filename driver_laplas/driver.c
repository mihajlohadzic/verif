...
static ssize_t laplas_read(struct file *pfile, char __user *buffer, size_t length, loff_t *offset)
{
    ...
    len = scnprintf(buff, BUFF_SIZE, "%d ",im_result[read_pos]);
    ...
    for (int i=border1 ;i< height-border1; i++)
    {
            for (int j=border1;j<(width-border1);j++)
            {
                ...
                {           
                    for (m=0;m<l1;m++)
                    { 
                        spix = spix+(mask1[k][m]*(picturEv2[i-border1+k][j-border1+m]));
    ...
    {    //j=0;
        ...
        for (int i = 0; i < height; ++i)
        {
            for (int j = 0; j < width; ++j)
            {
                ...
                if (im_result == 255)
                {
                    printk(KERN_INFO "ADRESA : position = %d\n", j+i*width);
                }
